...
<div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-3xl">
    ...
    <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div class="w-full px-5 py-6 bg-slate-100 border-2 border-slate-100 rounded-2xl flex items-center justify-center">

                @if($partner->logo_url)
                    <img src="{{ $partner->logo_url }}" class="h-16 object-contain">
                @else
                    <span class="text-slate-400 text-sm">Belum ada logo</span>
                @endif
